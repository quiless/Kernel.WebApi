...
namespace Kernel.WebApi.Upload
{
    ...
    public class FlowJsRepo : IFlowJsRepo
    {
        ...
        private FlowJsPostChunkResponse PostChunkBase(HttpRequest request, string folder, FlowValidationRules validationRules)
        {
            ...
            chunk.FileName = Guid.NewGuid().ToString() + System.IO.Path.GetExtension(chunk.FileName); //chunk.FileName.Split('.')[1];

            var response = new FlowJsPostChunkResponse { FileName = chunk.FileName, Identifier = chunk.Identifier, Size = chunk.TotalSize, OriginalFileName = file.FileName };
